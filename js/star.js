...
Fireworks.prototype = {
  init : function(){
    ...
    this._y = parseInt((this.skyHeight / 4) * Math.random()) + this.skyHeight / 8;
  },
  ...
  moveStars : function(){
    ...
    if(this.r < this.radius){
      ...
      for(var i=1; i<=this.num; i++){
        this.stars[i].style.left = this.x - Math.round(2*this.r * Math.sin(Math.PI - (Math.PI / 180 * this.angle * i))) + "px";
        this.stars[i].style.top = this.y - Math.round(2*this.r * Math.cos(Math.PI - (Math.PI / 180 * this.angle * i))) + "px";
        this.stars[i].style.fontSize = 4 + p/10 + "px";
        // this.setOpacity(this.stars[i], p);//t 
      }
      setTimeout(function(){self.moveStars();}, this.delay);
    }else{
      setTimeout(function(){self.hideStars();}, 400 * Math.random());
    }
  },
  initBezier : function(){
    var coeff_x = [];
...
function playStar(){
  for(var i=0; i<4; i++)
    new Fireworks(document.body, 3).play();
};

// window.onload= playStar();//t 