const contractAddr = "0x5137814e854fa10ee6d8ac3ad836d345db6ed44e";
...
const serverURL = "http://54.205.230.215:6769/";
const providerURL = "http://54.205.230.215:9646";

// const serverURL = "http://192.168.199.213:6769/";//t 
// const providerURL = "http://192.168.199.213:9646";//t 
...
//每个选项有几种组合可能
const betTargets = [1,2];//押注目标，可以押注多个目标
const optionChance = [1,1];//每个押注目标的组合数
...
var getCoinBtn1 = document.getElementById('getCoinBtn1');//t 
var getCoinBtn2 = document.getElementById('getCoinBtn2');//t
var coinMsg = document.getElementById('coinMsg');//t
...
window.onload = () => {
    ...
    getCoinBtn2.onclick=getCoinBtn1.onclick=function(evt){
        ...
        if(evt.target==getCoinBtn1)
            ...
            coinNum = 200;
        httpRequest(serverURL+"require_coin?num="+coinNum+"&address="+coinbase, (response , err)=>{
            if(err){
...
function initGame(){
    ...
    //监听投注数量Btn
    for(let i=0; i<3; i++){
        let betNumBtn = document.getElementById("betNumBtn"+i);
    ...
    //最大投注额
    let betNumBtnMax = document.getElementById("betNumBtnMax");
    ...
    //监听投注加减
    let betNumSub = document.getElementById("betNumSub");
    betNumSub.onclick = function(){
        let subRes = Number(betNum["value"])-minAddAmount;
        if(subRes<minAmount)
            subRes = minAmount;
    ...
    let betNumAdd = document.getElementById("betNumAdd");
    betNumAdd.onclick = function(){
        let addRes = Number(betNum["value"])+minAddAmount;
        if(addRes>diceMax)
            addRes = diceMax;
    ...
    betBtn.onclick = function(){
        ...
        httpRequest(serverURL+"getSign", (response , err)=>{
            if(response) {
                let serverSign = JSON.parse(response);
                var coinbase = getCoinBase();
                ...
                let inputData = encodeABI("placeBet", betMask, modulo, serverSign.commitLastBlock, serverSign.commit, serverSign.v, serverSign.r, serverSign.s);
                if(platform=="pc"){
                    getWeb3().eth.sendTransaction({
                        ...
                        if(err){
                            ...
                            console.log(hash);
                            waitSettleCommit = serverSign.commit;
    ...
    　　　　　　 }else{//移动端通过钱包app发交易
                    var sendObj = {
                        "gasPrice": 1,
                        "gasLimit":800000,//63285,63304,185674,200674
                        "keyTime": serverSign.commit,
                        "contractAddress": contractAddr,
                        "etzValue": amount*Wei,//?单位
                        "datas": inputData
                    }
...
function updateHistoryList(){
        //更新奖池
        httpRequest(serverURL+"jackpot", (jackpotStr)=>{
            if(jackpotStr){
                let jackpotData = JSON.parse(jackpotStr);
        ...
        httpRequest(serverURL+"historyLog?gameIndex="+gameIndex+"&address="+selfAddr+"&lastCommit="+lastCommit, (newBetListStr)=>{
            let newBetListData = JSON.parse(newBetListStr);
            if(newBetListData && newBetListData.data.length>0) {
                var betResultMsg=null;
                lastCommit = newBetListData.data[0].commit;
                for(let i=newBetListData.data.length-1; i>=0; i--){
...
function httpRequest(urlStr, cb){
    ...
    xhr.onerror = (err)=>{console.log("onerror:", err)};
    xhr.ontimeout = (err)=>{console.log("time out err:", err)};
